...
(function() {
  ...
  module.exports = {
    ...
    scanForWiFi: function() {
      const scanResults = this.execSync("nmcli -m multiline device wifi list", {timeout: 10000});
      const networks = [];
      const ref = scanResults.split('\n');
      let isKey = null;
      let isValue = null;
      ref.forEach((line, idx) => {
        line = line.replace("\n", "");
        line = line.replace(/[ ]{2,50}/gi, "");
        if ((/^SSID/).test(line)){
          const theSSID = line.replace("SSID:", "");
          if (theSSID !== "--"){
            networks.push(theSSID);
          }
        }
      });
      return networks;
